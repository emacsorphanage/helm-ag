...
(defun helm-ag--edit (_candidate)
  (with-current-buffer (get-buffer-create "*helm-ag-edit*")
    (erase-buffer)
    (let (buf-content helm-name)
      (with-current-buffer (get-buffer "*helm-ag*")
        (goto-char (point-min))
        (setq helm-name (helm-current-line-contents))
        (forward-line 1)
          ...
          (mapc (lambda (ov)
                  (when (eq 'helm-visible-mark (overlay-get ov 'face))
                    (push (buffer-substring-no-properties
                           (overlay-start ov) (overlay-end ov)) marked-lines)))
                (overlays-in body-start (point-max)))
          (if (not marked-lines)
              (setq buf-content (buffer-substring-no-properties
                                 body-start (point-max)))
            (setq buf-content (mapconcat 'identity marked-lines "\n")))))
      (insert buf-content)
